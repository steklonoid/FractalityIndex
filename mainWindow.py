...
class UiMainWindow(object):
    ...
    def setupui(self, mainwindow):
        ...
        self.operationmenu.addAction(self.loadbars)
        self.charcalc = QAction("Расчет характеристик", self)
        self.charcalc.triggered.connect(mainwindow.charcalc_triggered)
        self.operationmenu.addAction(self.charcalc)
        self.quantcalc = QAction("Расчет квантования MAXOF", self)
        self.quantcalc.triggered.connect(mainwindow.quantMAXOF_triggered)
        self.operationmenu.addAction(self.quantcalc)
        self.quantfrac = QAction("Расчет фрактальных a, b, sigma", self)
        self.quantfrac.triggered.connect(mainwindow.quantfrac_triggered)
        self.operationmenu.addAction(self.quantfrac)
        self.prepairNN = QAction("Подготовка данных для нейросети", self)
        self.prepairNN.triggered.connect(mainwindow.prepairNN_triggered)
        self.operationmenu.addAction(self.prepairNN)
        self.trainNN = QAction("Тренировка нейросети", self)
        self.trainNN.triggered.connect(mainwindow.trainNN_triggered)
        self.operationmenu.addAction(self.trainNN)

        self.secondpartmenu = self.menubar.addMenu('Анимация и др.')
        self.loadbars_2 = QAction("Загрузка баров", self)
        self.loadbars_2.triggered.connect(mainwindow.loadbars_2_triggered)
        self.secondpartmenu.addAction(self.loadbars_2)
        self.startanimation = QAction("Старт анимации", self)
        self.startanimation.triggered.connect(mainwindow.star_animation)
        self.secondpartmenu.addAction(self.startanimation)
